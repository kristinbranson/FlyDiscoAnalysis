function [datamerge,experiment_ids] = SAGEListBowlExperiments(varargin)

currdir = which('SAGEListBowlExperiments');
[currdir,~] = fileparts(currdir);

%% parse inputs
[docheckflags,daterange,SAGEpath,removemissingdata,dataset,data_type,rootdir,leftovers] = ...
...
allqueries = {SAGE.Query.Compare('experiment_name','=','FlyBowl*')};
...
if docheckflags,
  ...
  allqueries{end+1} = SAGE.Query.Any(SAGE.Query.Compare('flag_redo','=','None'),...
    SAGE.Query.Compare('flag_redo','=','0'),...
    SAGE.Query.Compare('screen_type','=','primary'));
...
if ~isempty(data_type),
  allqueries{end+1} = SAGE.Query.Compare('data_type','=',data_type);
end
...
bowlAssay_data = SAGE.Lab('olympiad');
bowlAssay_data = bowlAssay_data.assay('bowl');
bowlAssay_data = bowlAssay_data.dataSet(dataset);
