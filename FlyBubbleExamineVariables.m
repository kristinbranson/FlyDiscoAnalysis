...
params.rootdatadir = '/tier2/branson/fly_bubble/bubble_data';
% date range stuff
params.date = struct;
...
if ~iscell(data.data_types),
  switch lower(params.vartype),
    case 'experiment',
      data.data_types = {'ctrax_diagnostics_*','registrationdata_*'};
      %'ufmf_diagnostics_summary_*','sexclassifier_diagnostics_*','bias_diagnostics_*','temperature_diagnostics_*','bkgd_diagnostics_*'
...
%% load data from SAGE or cached mat file
  ...
  function success = GetData(varargin)
    ...
    elseif nargin >= 1,
      ...
      data.queries(end+1:end+2) = {'experiment_name','FlyBubble_*'};
      data.pull_data_datetime = now;
      rawdata = SAGEGetBubbleData(data.queries{:},'checkflags',params.docheckflags,'removemissingdata',true,'dataset',data.dataset,'analysis_protocol',params.analysis_protocol,...
    ...
    for tmpi = 1:data.nexpdirs,
      data.expdir_bases{tmpi} = regexprep(rawdata(tmpi).experiment_name,'^FlyBubble_','');
