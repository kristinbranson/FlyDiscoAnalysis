...
my $nargs = $#ARGV + 1;
if($nargs < 1){
    print "Usage: fork_FlyBowlClassifySex.pl <expdirlist.txt>\n";
    exit(1);
}

my $ANALYSIS_PROTOCOL = "20110407";

my $SCRIPT = "/groups/branson/bransonlab/projects/olympiad/FlyBowlAnalysis/settings/$ANALYSIS_PROTOCOL/run_FlyBowlClassifySex.sh";
...
while(my $expdir = <FILE>){
    ...
    my $sgeid = "kb_flybowlclassifysex_$basename";
    $sgeid =~ s/\//_/g;
    $sgeid =~ s/\./_/g;
    $sgeid =~ s/\;/_/g;
...
# FlyBowlClassifySex test script.
