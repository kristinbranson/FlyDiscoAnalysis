...
elseif numel(x) == t
    ...
    error(message('bioinfo:masmooth:XYmustBeSameLength'));
end
...
elseif span <= 0  % span must be positive
    error(message('bioinfo:masmooth:SpanMustBePositive')); 
...
if nargin<4 || isempty(method)
    ...
    switch method
        ...
        case 'sgolay'
            runSgolay = true;
        otherwise
            error('bioinfo:masmooth:UnknownMethod',...
...
if runSgolay
    if nargin>5
        error(message('bioinfo:masmooth:TooManyInputsForSgolay'))
    end
...
if (weighting == 2) && (span-1 <= order)
    span = order+2;
    warning('bioinfo:masmooth:SmallSpan',...
...
for k = 1:robust
    ...
    for i=1:n
        ...
        % outweight outliers
        mar = median(abs(rw)); % Median of the abosulute residuals
        if mar > maxabsyXeps
            rw = 1-(rw./(6*mar)).^2; % Bi-squared weight function
            rw(rw<0) = 0;
            weight = weight .* rw; 
        end
...
% check for NaNs in y and take appropriate measures.
%
