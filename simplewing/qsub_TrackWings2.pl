...
my $SETTINGSDIR = "/groups/branson/bransonlab/projects/olympiad/FlyBowlAnalysis/settings";

if($nargs >= 2){
    $ANALYSIS_PROTOCOL = $ARGV[1];
}

my $SCRIPT = "/groups/branson/home/bransonk/tracking/code/JCtrax/simplewing/TrackWings/distrib/run_$SCRIPTNAME.sh";

my $PARAMS = "paramsfile $SETTINGSDIR/$ANALYSIS_PROTOCOL/WingTrackingParameters.txt";
...
while(my $expdir = <FILE>){
    ...
    # submit command
    my $cmd = qq~qsub -N $sgeid -j y -b y -o $outfilename -cwd  -l new=true,archive=true $shfilename~;
