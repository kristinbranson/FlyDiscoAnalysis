...
[analysis_protocol,settingsdir,datalocparamsfilestr,visible,DEBUG,...
  ...
  plotflies,plotstimtrajs,verbose,stimcolor] = ...
  myparse(varargin,...
...
if plotstim > 0,
  ...
  for fi = 1:numel(stimulus_plotparams.features),
    ...
    for ion = 1:non,
      ...
      if all(stimulus_plotparams.stimcolor == 'green');
          stimcolors = struct('stimbkgd',[0.83 0.97 0.56],...
              'stim',[0.04 0.4 0.04],...
              'off',[0 0 0],...
              'stimstd',[0.2 0.8 0.2],...
              'offstd',[.7,.7,.7]);
      elseif all(stimulus_plotparams.stimcolor == 'red');
          stimcolors = struct('stimbkgd',[0.927 0.8156 0.8368],...
