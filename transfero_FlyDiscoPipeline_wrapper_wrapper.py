...
def transfero_FlyDiscoPipeline_wrapper_wrapper(raw_experiment_folder_path, user_name_for_configuration_purposes):
    ...
    with cd(this_script_folder_path) as _ :
        ...
        #os.environ['FONTCONFIG_USE_MMAP'] = 'false'  # to hopefully eliminate intermittent bus error  # using Matlab 2024b should fix
        command_line_as_list = ['/usr/bin/xvfb-run', '-d', '/misc/local/matlab-2024b/bin/matlab', '-batch', matlab_command_line]
