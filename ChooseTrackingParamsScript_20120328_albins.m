...
rootdir_fixed = '';
rootoutputdir = '/groups/branson/bransonlab/projects/olympiad/FlyBowlCtrax/20120328_non_olympiad_albins/results';

analysis_protocol = '20120328_non_olympiad_albins';
...
analysis_protocol0 = '20120210';
settingsdircurr = fullfile(settingsdir,analysis_protocol);
if ~exist(settingsdircurr,'dir'),
  settingsdir0 = fullfile(settingsdir,analysis_protocol0);
  pwdprev = pwd;
  mkdir(settingsdircurr);
  cd(settingsdircurr);
  files = dir(settingsdir0);
  for i = 1:numel(files),
    if ismember(files(i).name,{'.','..','expdirs.txt'}),
      continue;
    end
    if files(i).name(end) == '~',
      continue;
    end
    cmd = sprintf('ln -s ../%s/%s',analysis_protocol0,files(i).name);
    unix(cmd);
  end
  cd(pwdprev);
end
...
% % required files
% % experiment_params.subreadfiles = {'test_ctrax_results.mat'};
% % what lines
...
for i = 1:numel(experiments_all),
  ...
  annfile = fullfile(rootoutputdir,experiment_name,annfilestr);
...
thresh_prctiles_area = [.2,100-.02];
thresh_prctiles_a = [.23,100-.01];
thresh_prctiles_b = [.18,100-.05];
thresh_prctiles_ecc = [.05,100-.5];
...
% 20120328_non_olympiad_albins
% min_area = 69.408, mean_area = 118.884, max_area = 171.559
% min_a = 3.720, mean_a = 5.256, max_a = 6.519
% min_b = 1.394, mean_b = 1.792, max_b = 2.248
% min_ecc = 0.279, mean_ecc = 0.342, max_ecc = 0.418

% albins 20120110
% min_area = 70.477, mean_area = 121.651, max_area = 170.306
% min_a = 3.921, mean_a = 5.313, max_a = 6.416
% min_b = 1.335, mean_b = 1.816, max_b = 2.213
% min_ecc = 0.265, mean_ecc = 0.343, max_ecc = 0.432

% 20120228_housing_CS
% min_area = 50.210, mean_area = 89.400, max_area = 125.569
% min_a = 2.948, mean_a = 4.577, max_a = 5.491
% min_b = 1.189, mean_b = 1.551, max_b = 2.139
% min_ecc = 0.266, mean_ecc = 0.340, max_ecc = 0.460
