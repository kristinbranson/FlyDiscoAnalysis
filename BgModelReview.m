...
% Last Modified by GUIDE v2.5 12-Nov-2010 04:43:16
...
% TODO: change this to the path to the bowl data
handles.rootdir = '';
...
handles.menuShowLabels = ...
  [handles.var2ShowLabel(:,2)',...
  {'Current Frame','Difference Image','Is Foreground','Connected Components'}];
...
if ~isfield('background_center',handles.ann),
  handles.ann.background_center = handles.ann.background_median;
end
if ~isfield('background_dev',handles.ann),
...
handles.showType = 'Background Median';

% initialize current colormap 
...
border = .01;
handles.colorbarPos = [axespos(1)+axespos(3)+border,axespos(2),1-(axespos(1)+axespos(3))-border*2,axespos(4)];
handles.hColorbar = colorbar('peer',handles.axes_Video,'East');
set(handles.hColorbar,'Position',handles.colorbarPos);
...
for i = 1:length(handles.menuShowLabels),
  ...
  j = find(strcmp(label,handles.var2ShowLabel(:,2)),1);
...
switch handles.showType,
  case handles.var2ShowLabel(:,2)',
    i = find(strcmp(handles.showType,handles.var2ShowLabel(:,2)),1);
    ...
    if isfield(handles.ann,annfn),
      ...
      if ismember(handles.showType,handles.showRangeIs0to255),
        ...
        set(handles.axes_Video,'Clim',[min(handles.ann.(annfn)(:)),max(handles.ann.(annfn)(:))]);
      end
    else
