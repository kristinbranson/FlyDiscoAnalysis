...
% Specify the analysis test folder
this_script_path = mfilename('fullpath') ;
this_folder_path = fileparts(this_script_path) ;
analysis_test_folder_path = fullfile(this_folder_path, 'analysis-test-folder') ;
...
if do_transfer_data_from_rigs ,
    command_line = sprintf('scp -B -r %s %s@%s:%s', analysis_test_folder_path, remote_user_name, remote_host_name, remote_user_folder_path) ;  %#ok<UNRCH>
else
    analysis_test_folder_destination_path = fullfile(destination_folder_path, 'taylora') ;
    ensure_folder_exists(analysis_test_folder_destination_path) ;
    command_line = sprintf('cp -R %s/* %s', analysis_test_folder_path, analysis_test_folder_destination_path) ;
end
...
analysis_test_folder_destination_path = fullfile(destination_folder_path, 'taylora') ;
local_verify(analysis_test_folder_path, analysis_test_folder_destination_path) ;
