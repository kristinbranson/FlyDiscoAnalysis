function handles = PlotPerFrameStatsExp(stats_perframefeatures,statsperexp,meanstatsperexp,controlstats,basename,varargin)
...
nprctiles = numel(meanstatsperexp.(fns{1}).meanprctiles);
...
for fni = 1:nfns,
  ...
  if isempty(controlstats),
    ...
    goodidx = ~isnan(controlstats.statsperexp.(fn).Z) & controlstats.statsperexp.(fn).Z > 0;
    nexpscontrol = nnz(goodidx);
    stderrmean_control(fni) = 1/sqrt(nexpscontrol);
  end

  meanmeans(fni) = (meanstatsperexp.(fn).meanmean - mu) / sig;
  stdmeans(fni) = meanstatsperexp.(fn).stdmean / sig;
  meanprctiles(:,fni) = (meanstatsperexp.(fn).meanprctiles - mu) / sig;
  stdprctiles(:,fni) = meanstatsperexp.(fn).stdprctiles / sig;
  
  goodidx = ~isnan(statsperexp.(fn).Z) & (statsperexp.(fn).Z > 0);
  nexps = nnz(goodidx);
  nexpsanalyzed(fni) = nexps;
  stderrmean(fni) = stdmeans(fni) / sqrt(nexps);
  stderrprctiles(:,fni) = stdprctiles(:,fni) / sqrt(nexps);
