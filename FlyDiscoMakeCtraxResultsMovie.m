...
%           indicatorframes(2*step)=indicatorframes(2*step-1);
    ...
    if exist(indicatorfile,'file')
      ...
      if ~all(is_local_indicatorframes_valid) ,
          warning(['The largest value in local_indicatorframes (%d) is too big for indicatorLED.startframe (length = %d).  ' ... 
                   'Likely there was a problem with indicator detection.  Trimming...'], ...
