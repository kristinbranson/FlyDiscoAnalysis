...
if plotflies,
  ...
  position_fly(4) = min(ss(4),position(4)*nflies);
  clf(hfigfly);
  set(hfigfly,'Units','pixels','Visible',visible,'Position',position_fly);
  hax_fly = createsubplots(nflies,1,[[.05,.05];[.05,.01]],hfigfly);

  for fly = 1:nflies,
  
    patch(hax_fly(fly),[t0,t0,t1,t1,t0],ylim([1,2,2,1,1]),colors.stimbkgd,'LineStyle','none');
    hold(hax_fly(fly),'on');
  
    plot(hax_fly(fly),trx.movie_timestamps{1}(fpre:f0-1),data(fly,1:f0-fpre),'-','LineWidth',1,'Color',colors.off);
    plot(hax_fly(fly),trx.movie_timestamps{1}(f1+1:fpost),data(fly,f1-fpre+2:end),'-','LineWidth',1,'Color',colors.off);
    plot(hax_fly(fly),trx.movie_timestamps{1}(f0:f1),data(fly,f0-fpre+1:f1-fpre+1),'-','LineWidth',1,'Color',colors.stim);
    set(hax_fly(fly),'YLim',ylim,'XLim',[tpre,tpost]);
    
    ylabel(hax_fly(fly),sprintf('Fly %d',fly),'Interpreter','none');
    if fly == nflies,
      xlabel(hax_fly(fly),'Time (s)');
    else
      set(hax_fly(fly),'XTickLabels',{});
    end
    if fly == 1,
      title(hax_fly(fly),{basename,sprintf('%s, period %d',field,ion)},'Interpreter','none');
    end
  end
