function handles = PlotPerExpHists(field,hist_perframefeatures,...
  meanhistperexp,histperexp,bininfo,hist_plot_params,expname,varargin)
...
[centers,edges,meanfrac,stdfrac,stderrfrac,nexpsanalyzed,Zpertype] = ...
  SelectHistDataExp(fns,bininfo,hist_plot_params,plottype,...
...
for typeii = 1:ntypes,
  ...
  if strcmpi(stdstyle,'patch'),
    hstd(typeii) = patch([centers,fliplr(centers)],...
      [meanfrac{typei}+stdfrac{typei},...
      fliplr(meanfrac{typei}-stdfrac{typei})],...
      typecolor*stdalpha + (1-stdalpha),'LineStyle','none','parent',hax);
  else
    hstd(typeii) = errorbar(centers,meanfrac{typei},stdfrac{typei},'.',...
