function handles = PlotPerFrameStatsComparisonExp(stats_perframefeatures,statsperexp,meanstatsperexp,controlstats,basenames,varargin)

% parse plotting parameters
[hfig,visible,position,axposition,allfields,...
  allframeconditions,allflyconditions,plotstderr] = ...
...
for fni = 1:nfns,
  ...
  if isempty(controlstats),
    ...
    mu = controlstats.meanstatsperexp.(fn).meanmean;
    sig = controlstats.meanstatsperexp.(fn).stdmean;
    goodidx = ~isnan(controlstats.statsperexp.(fn).Z);
    nexpscontrol = nnz(goodidx);
    stderrmean_control(fni) = 1/sqrt(nexpscontrol);
  end

  for datai = 1:ndata,
    meanmeans(datai,fni) = (meanstatsperexp{datai}.(fn).meanmean - mu) / sig;
    stdmeans(datai,fni) = meanstatsperexp{datai}.(fn).stdmean / sig;
    goodidx = ~isnan(statsperexp{datai}.(fn).Z) & statsperexp{datai}.(fn).Z > 0;
    nexps = nnz(goodidx);
    nexpsanalyzed(datai,fni) = nexps;
    stderrmean(datai,fni) = stdmeans(datai,fni) / sqrt(nexps);
  end
end
...
if plotstderr,
  ...
  maxy = max(meanmeans(:)+stdmeans(:));
  miny = min(meanmeans(:)-stdmeans(:));
end
...
if plotstderr,
  hti = title(hax,sprintf('Mean, stderr of per-experiment mean'),'Interpreter','none');
else
  hti = title(hax,sprintf('Mean, std of per-experiment mean'),'Interpreter','none');
end
