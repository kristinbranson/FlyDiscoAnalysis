function do_continue_pipeline = CheckACIResultsFileContents(expdir, dataloc_params)
    ...
    if ~exist(aci_results_as_text_file_path, 'file')
        fprintf('ACI results file %s does not exist\n', aci_results_as_text_file_name) ;
    ...
    if isfield(aci_results, 'automated_pf') ,
        ...
        elseif strcmp(aci_results.automated_pf, 'F') ,
            ...
            fprintf('ACI results file %s has automated_pf value that is not P, nor F, nor U\n', aci_results_as_text_file_name) ;
            do_continue_pipeline = false ;
        end
    else
