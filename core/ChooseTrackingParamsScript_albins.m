...
trxfilestr = 'new_ctrax_results.mat';

experiment_params = struct;
% check for failures
experiment_params.checkflags = false;
% check for aborted
experiment_params.flag_aborted = '0';
% experimenter
experiment_params.experimenter = 'albins';
% root data dir
experiment_params.rootdir = rootdir;
% what dates should we analyze
experiment_params.daterange = {'20110425T000000','20110719T000000'};
% what lines
...
istracked = cellfun(@(s) exist(fullfile(s,trxfilestr),'file'), {experiments_all.file_system_path}) ~= 0;
experiments_all = experiments_all(istracked);

[experiments,ngal4_chosen,ncontrols_chosen] = choose_expdirs(experiments_all,ngal4,ncontrols,'doseparatecontrols',false);
...
trx = [];
for i = 1:nexpdirs,
  trxfile = fullfile(expdirs{i},trxfilestr);
...
thresh_prctiles_area = [.25,100-.05];
...
% 20110725
% min_area = 85.000, mean_area = 122.559, max_area = 160.000
% min_a = 4.365, mean_a = 5.335, max_a = 6.250
% min_b = 1.450, mean_b = 1.818, max_b = 2.125
% min_ecc = 0.285, mean_ecc = 0.341, max_ecc = 0.400

% 20110722
% min_area = 83.606, mean_area = 122.877, max_area = 159.285
% min_a = 4.350, mean_a = 5.363, max_a = 6.375
% min_b = 1.425, mean_b = 1.818, max_b = 2.150
% min_ecc = 0.275, mean_ecc = 0.340, max_ecc = 0.400
...
% 20110725
% Constant velocity center position dampening: 0.137965
% Constant velocity angle dampening: 0.464267

% 20110722
% Constant velocity center position dampening: 0.168290
% Constant velocity angle dampening: 0.519836
...
% 20110725
% Weight of angle in matching criterion: 146.481056

% 20110722
% Weight of angle in matching criterion: 158.506951
...
%% choose some videos to retrack

median_area_per_exp = nan(1,nexpdirs);
for i = 1:nexpdirs,
  idx = [trx.expdiri] == i;
  area = [trx(idx).a_mm].*[trx(idx).b_mm];
  median_area_per_exp(i) = median(area);
end
[~,order] = sort(median_area_per_exp);

nretrack = 8;
idx_retrack = order(round(linspace(1,nexpdirs,nretrack)));
experiments_retrack = experiments(idx_retrack);
expdirs_retrack = {experiments_retrack.file_system_path};

for i = 1:nretrack,
  [~,s] = fileparts(expdirs_retrack{i});
  fprintf('%s\n',s);
end